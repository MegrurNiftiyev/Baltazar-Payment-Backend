...
const tokenizeSchema = z.object({
  cardNumber: z
    .string({ required_error: 'cardNumber is required' })
    .regex(/^\d{16}$/, 'cardNumber must be exactly 16 digits'),
  cardHolder: z
    .string({ required_error: 'cardHolder is required' })
    .min(1, 'cardHolder cannot be empty'),
  expiryMonth: z
    .string({ required_error: 'expiryMonth is required' })
    .regex(/^(0[1-9]|1[0-2])$/, 'expiryMonth must be 2 digits (01-12)'),
  expiryYear: z
    .string({ required_error: 'expiryYear is required' })
